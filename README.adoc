...
image:https://github.com/jiro4989/relm/workflows/test/badge.svg[test]
image:https://codecov.io/gh/jiro4989/relm/branch/master/graph/badge.svg[codecov, link="https://codecov.io/gh/jiro4989/relm"]

`relm` は GitHub Release からインストールしたコマンド類を統一的なインタフェー
...
ルに記載のパスにインストールされます。初期設定では `~/relm/bin` にシンボリック
リンクが作成されます。
それらの設定ファイルの実体は `~/relm/pkg/<owner>/<repo>` に保存されます。
...
----
relm init
relm install https://github.com/itchyny/mmv/releases/download/v0.1.2/mmv_v0.1.2_linux_amd64.tar.gz
----

インストールされたコマンドとパッケージの情報は `~/relm/pkg/<owner>/<repo>` に
...
[source,bash]
----
relm upgrade itchyny/mmv
----

これは `relm upgrade <owner>/<repo>` というルールに従っています。
バージョンを指定したい場合は以下のコマンドを実行します。

[source,bash]
----
relm upgrade itchyny/mmv v0.1.2
----
...
[source,bash]
----
relm remove --yes itchyny/mmv

# または

relm remove -y itchyny/mmv
----
